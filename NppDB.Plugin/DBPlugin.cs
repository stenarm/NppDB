    ...
    public class NppDbPlugin : PluginBase, INppDbCommandHost
    {
        ...
        private void ReadTranslations()
        {
            ...
            var dict = new Dictionary<ParserMessageType, string>
            {
                ...
                { ParserMessageType.MULTIPLE_WHERE_USED,
                    "WHERE used multiple times" },
                { ParserMessageType.MISSING_EXPRESSION_IN_JOIN_CLAUSE,
                    "Missing expression in JOIN clause" },
                { ParserMessageType.UNNECESSARY_SEMICOLON,
                    "Unnecessary semicolon" },
                { ParserMessageType.MISSING_EXPRESSION_IN_WHERE_CLAUSE,
                    "Missing expression in WHERE clause" },
                { ParserMessageType.TOP_KEYWORD_MIGHT_RETURN_MULTIPLE_ROWS,
                    "TOP keyword might return multiple rows" },
                { ParserMessageType.POSSIBLE_NON_INTEGER_VALUE_WITH_TOP,
                    "Limit value used with TOP might not be an integer" },
                { ParserMessageType.TOP_LIMIT_CONSTRAINT,
                    "Limit value with TOP must be larger than zero" },
                { ParserMessageType.TOP_LIMIT_PERCENT_CONSTRAINT,
                    "Limit value with TOP PERCENT must be between zero and one hundred" },
                { ParserMessageType.ONE_ROW_IN_RESULT_WITH_TOP,
